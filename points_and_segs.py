    ...
    def x_intercept(self, y_value):
        if self.contains_y(y_value):
            if self.y_vals[0]>self.y_vals[1]:
                # need to flip the order of points if y values are decending
        ...
        raise Exception("Module points_and_segs: No y intercept found.  "
...
class Garment(Shape):
    # assumes there is a dict called self.pattern_pieces of garment style pattern pieces
    def add_to_subplot(self, subplot):  # sets artists for body plots
        for piece in self.pattern_pieces:
            line_color = "magenta"
            line_style = "--"
            if self.pattern_pieces[piece]!=None:
                if piece == "front":
                    line_color = "blue"
                    line_style = ":"
                if piece == "back":
    ...
    def write_instructions(self, piece):
        ...
        # TODO keep track of knitting in the case of a split
        ...
        for row in range(0, total_rows):  # iterates through each row
            ...
            if leftmost_needle == last_leftmost_needle and rightmost_needle == last_rightmost_needle:
                ...
                if row % 2 != 0:  # if the row number is odd
                    ...
                    carriage_position, carriage_direction = cp[1], cd[1]  # and carriage starts on the right# compare if this is the first row of a split
                    print(f"Continue knitting until row counter reads {row}:")
                    print("")
                if len(needles)==2:
                    split = False
                if len(needles)>2 and split==False:
                    split_row=True
        ...
        if split == True:
            lines = tr.wrap(f"Cast off remaining stitches.  Reset counter to {split_counter}, and complete "
                            f"opposite side, reversing left and right intructions.", width=width)
            for line in lines: print(line)
...
class Tshirt(Garment):
    ...
    # def debug_find_x_intercepts(self):
    #     """
    #     to find duplicates in an array use np.unique with return_counts argument:
    #     list_of_unique_values, occurrences = np.unique(some_array, return_counts=True)
    #
    #     below, I will use it this way:
    #     stitches, counts = np.unique(stitch_positions, return_counts=True)
    #     in cases where there is more than one segment containing a particular y value:
    #
    #                             HORIZONTAL SEGMENTS - single duplicates and odd len(stitch_positions)?
    #     if a segment is horizontal, there is a cast on or cast off situation
    #     and there will be a set of duplicate x intercepts in the counts array
    #     EXAMPLE:    at the cast on edge we see 3 values in
    #                 stitch_positions [-23.275, 23.275, -23.275]
    #                 with counts array([2, 1])
    #                 - there are 2 unique values, one of which appears twice
    #     EXAMPLE:    at the neckline horizontal split we see 5 values in
    #                 stitch_positions [-23.666666666666668, -9.5, 9.5, 9.5, 23.666666666666668]
    #                 with counts - array([1, 1, 2, 1])
    #                 - there are 4 unique values, one of which appears twice
    #     there is a 2 in each counts array which means a value occurs twice - there's a duplicate
    #     when there is a split with a horizontal line, there will be 4 unique values with one duplicate
    #     QUESTION: what if there are two horizontal segments like at the armhole?????
    #
    #                             INFLECTION POINTS - double duplicates and even len(stitch_positions)?
    #     when there is an inflection point, there will be two sets of duplicate x values
    #     when there is an inflection point and a split, there will be 4 unique values with two duplicates
    #     EXAMPLE:    at the fullBust inflection we see 4 values in
    #                 stitch_positions [-23.037499999999998, -23.037499999999998, 23.037499999999998, 23.037499999999998]
    #                 with counts array([2, 2])
    #                 - there are 2 unique values - each which appear twice
    #     EXAMPLE:    at the outerShoulder inflection we see 6 values in
    #                 stitch_positions [-24.0, -24.0, -9.5, 9.5, 24.0, 24.0]
    #                 with counts array([2, 1, 1, 2])
    #                 - there are 4 unique values, two of which appear twice
    #     this is because there are needles out of work for the neck as well as needles on hold for the other shoulder
    #
    #                             SPLITS - even len(stitch_positions)
    #     how to tell if there is a split? if there is a split, there will be an even number of unique intercepts
    #
    #     """
    #     # print(f"the minimum y value in the front piece is {min(self.pattern_pieces["front"].y_vals)}")
    #     for y in range(min(self.pattern_pieces["front"].y_vals), max(self.pattern_pieces["front"].y_vals)):
    #         seg_list = []
    #         row = y - min(self.pattern_pieces["front"].y_vals)
    #         stitch_positions = []
    #         for segment in self.pattern_pieces["front"].segments:
    #             if segment.contains_y(y):
    #                 seg_list.append(segment)
    #                 stitch_positions.append(np.trunc(segment.x_intercept(y)))
    #                 # print(f"row {row}: segment coordinates are: {segment.seg}, "
    #                 #       f"and its x value is {segment.x_intercept(y)} at y = {y}.")
    #         if len(seg_list) > 2:
    #             # print(f"there are {len(seg_list)} segments containing {y} in row {row} with x values of "
    #             #       f"{stitch_positions}")
    #             stitches, counts = np.unique(stitch_positions, return_counts=True)
    #             print(f"in row {row}, there are {len(stitch_positions)} values in stitch_positions {stitch_positions} "
    #                   f" with duplicate counts of {counts}")
    #             if (len(stitch_positions) % 2) != 0:
    #                 print(f"{len(stitch_positions)} is an odd number.  I think there is a horizontal segment here.")
    #                 if len(stitches) > 2:
    #                     print(
    #                         f"there are more than 2 unique x values in {stitches}.  I think there is also a split here.")
    #             else:
    #                 if max(counts) < 2:
    #                     print(f"{len(stitch_positions)} is an even number, but there are no "
    #                           f"duplicate values . I think there is a split here.")
    #                 else:
    #                     print(f"{len(stitch_positions)} is an even number. I think there is an inflection point here.")
    #
    #             breakpoint()
class Tshirt_fitted(Garment):
    def __init__(self, body, person):
        ...
        self.style = "Fitted T Shirt"
        # create the tshirt front piece from the body data
        piece_points = None
        self.front_places_with_ease = {
            "highHip": -5,
            "waist": 10,
